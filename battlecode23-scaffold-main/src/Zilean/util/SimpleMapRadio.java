...
public class SimpleMapRadio {
    ...
    public boolean writeBlock(MapPckg pckg){
        try{
    ...
    public List<MapPckg> readAndCacheEmpty() throws GameActionException {
        emptyArraySlots = new LinkedList<>();
        List<MapPckg> mapDiff = new ArrayList<>(32);
        for(int i = 0; i < MAP_VOLUME_SIZE; i++){
            int serialized = rc.readSharedArray(i + MAP_INDEX_OFFSET);
            MapPckg pckg = new MapPckg(serialized);
            if(isMsgValid(pckg)){
                mapDiff.add(pckg);
            } else {
    ...
    boolean isMsgValid(MapPckg pckg){
