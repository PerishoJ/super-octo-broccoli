...
public class MappingRadio_old {
    ...
    private LocalMap_Old.MapCell[] blockData;
    public MappingRadio_old(RobotController rc){
        this.rc = rc;
     ...
     */
    public List<LocalMap_Old.MapCellAndLoc> groomMapVolume(int turn) throws GameActionException {
        List<LocalMap_Old.MapCellAndLoc> fullVolume = new ArrayList<>(MAPPING_VOLUME_SIZE);
        for(int blockNumber = 0 ; blockNumber < NUMBER_OF_BLOCKS ; blockNumber++) {
            if(shouldCleanBlock(blockNumber,turn)){ // after grace period, delete full blocks
                ...
                List<LocalMap_Old.MapCellAndLoc> block = readBlock(blockNumber);
    ...
    public List<LocalMap_Old.MapCellAndLoc> readBlock(int blockNumber) throws GameActionException {
        //exception handling
        ...
        List<LocalMap_Old.MapCellAndLoc> block = new ArrayList<>(MESSAGES_PER_BLOCK);
    ...
    public List<LocalMap_Old.MapCellAndLoc> readVolume() throws GameActionException {
        //figure out where the block is, grab the map info and location for each valid place you can.
        List<LocalMap_Old.MapCellAndLoc> block = new ArrayList<>(MESSAGES_PER_BLOCK);
        int blockOffset = 0;
    ...
    private void readCellFromSharedArray(List<LocalMap_Old.MapCellAndLoc> block, int blockOffset, int msgIndex) throws GameActionException {
        ...
        boolean isCellGood = LocalMap_Old.MapCell.isValidMessage(compressedMapCell); // very quickly tell if the message is any good.
        if(isCellGood){
            LocalMap_Old.MapCellAndLoc mapCellAndLoc = new LocalMap_Old.MapCellAndLoc();
            mapCellAndLoc.cell = new LocalMap_Old.MapCell(compressedMapCell); //deserialize fully if good msg
    ...
    private void cleanBlock(int blockNumber) throws GameActionException {
        List<LocalMap_Old.MapCell> block = new ArrayList<>(MESSAGES_PER_BLOCK);
        int blockOffset = blockNumber * MESSAGES_PER_BLOCK * MAP_MESSAGE_SIZE;
        for (int i = 0; i < MESSAGES_PER_BLOCK; i++) {
