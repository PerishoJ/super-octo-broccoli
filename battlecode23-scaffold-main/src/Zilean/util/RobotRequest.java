...
public class RobotRequest {
    ...
    public RequestType getType(){
        boolean isValidEnum = metadata[0] < RequestType.values().length && metadata[0] >= 0;
        if(isValidEnum) {
           return RequestType.values()[metadata[0]];
        } else {
           return RequestType.BAD_MESSAGE;
        }
    }

    public void setType(RequestType type){
        metadata[0] = type.ordinal();
    }
    /**
    ...
    /**
     * we use metadata[0] for Request type now...but we still return it in metadata to support legacy code. Sorry.
     * @param mtData
     */
