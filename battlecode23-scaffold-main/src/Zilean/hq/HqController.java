...
import Zilean.scouting.ScoutingUtils;
import Zilean.util.*;
...
public class HqController {
    ...
    public void run (RobotController rc, int turnCount) throws GameActionException {
        ...
        List<MapPckg> mapDIff = mapRadio.readAndCacheEmpty();
    ...
    private void handleNewMapFeatures(RobotController rc, List<RobotRequest> requests, MapPckg pckg) throws GameActionException {
    ...
    private void buildOrder(RobotController rc, int turnCount) throws GameActionException {
        if(turnCount == 1){
            ...
            mapRadio.writeBlock(new MapPckg(MapFeature.OUR_HQ , rc.getLocation()));
        }
        //make some launchers
    ...
    private void scanForNewMapFeatures(RobotController rc, List<MapPckg> mapDIff, List<RobotRequest> requests) throws GameActionException {
        //Handle any new Map features coming in off the line
        if(!mapDIff.isEmpty()){
            for(MapPckg pckg : mapDIff){
    ...
    private static boolean isMiningRequestAlreadyInSharedArray(List<RobotRequest> requests, MapPckg pckg) {
    ...
    private static boolean isThisAResourceWell(MapPckg pckg) {
